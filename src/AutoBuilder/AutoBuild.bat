...
::重新编译生成文件
"C:\Program Files (x86)\MSBuild\14.0\Bin\MSBuild.exe" ../Owen.Site.Host\Owen.Site.Host.csproj /p:Configuration=Release;OutDir=%outputDir%\OutPut\bin /t:Rebuild 
...
::复制host工程中的一些文件或者某些文件夹下面的所有文件（）
for /f %%i in (HostMustCopyFileList.txt) do (HostMustCopyFileList.txt在里面配制)
